...
import { createImageUpload, handleImageDrop, handleImagePaste, UploadImagesPlugin } from './image-upload'
...
export const Image = TiptapImage.extend<IImageOptions>({
  ...
  addAttributes() {
    return {
      ...
      border: {
        ...
        parseHTML: element => element.style.border || '2px solid red',
        renderHTML: (attributes) => {
          return {
      ...
      borderColor: {
        default: 'red',
        parseHTML: element => element.style.borderColor || 'red',
        renderHTML: (attributes) => {
          return {
  ...
  renderHTML({ HTMLAttributes }) {
    const { flipX, flipY, align, inline, border } = HTMLAttributes
    ...
    return [
      inline ? 'span' : 'div',
      {
        style: `${textAlignStyle} ${borderStyle}`,
        class: 'image',
      },
      [
        'img',
